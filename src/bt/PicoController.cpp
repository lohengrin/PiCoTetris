...
void Controller::setPlayerLEDs(uint8_t led) const {
    ...
    if (pico_set_player_leds(_idx, led) == -1)
...
void Controller::setColorLED(uint8_t red, uint8_t green, uint8_t blue) const {
    ...
    if (pico_set_lightbar_color(_idx, red, green, blue) == -1)
...
void Controller::setRumble(uint8_t force, uint8_t duration) const {
    ...
    if (pico_set_rumble(_idx, force, duration) == -1)
...
void Controller::onConnected() {
    ...
    if (pico_get_controller_properties(_idx, &_properties) != UNI_PICO_OK) {
